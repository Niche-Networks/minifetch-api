...
config({ path: envPath });
// Set network
const networkPrefixArg = args.find(arg => arg.startsWith("--network="));
const networkPrefix = networkPrefixArg.split("=")[1].toUpperCase();
console.log(networkPrefix);
const network = process.env[`${networkPrefix}_NETWORK`];
console.log("using network: " + network);
// Derive x402 signer account from private key
let privateKey;
let signer;
if (network.startsWith("solana")) {
    privateKey = process.env[`${networkPrefix}_PRIVATE_KEY`];
...
const explorerBaseUrl = process.env[`EXPLORER_${networkPrefix}_URL`];
...
const endpointPath = endpoint === "check"
    ? process.env.CHECK_ROBOTS_API_PATH
    : process.env.METADATA_API_PATH;
// Rest of the querystring params
const metadataUrl = process.env.METADATA_URL; // url to fetch metadata from
const boolInclRespBody = process.env.METADATA_API_PARAM_INCLUDE_RESPONSE_BODY;

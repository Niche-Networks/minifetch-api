...
describe.sequential("x402: checkAndExtractUrlPreview() e2e", { timeout: 30000 }, () => {
  ...
  it("base-sepolia success", async () => {
    ...
    const response = await client.checkAndExtractUrlPreview("https://minifetch.com");
...
describe.sequential("x402: checkAndExtractUrlPreview() handles errors gracefully", { timeout: 30000 }, () => {
  ...
  it("throws w bad private key", async () => {
    ...
    const r = await expect(failClient.checkAndExtractUrlPreview("https://anthropic.com")).rejects.toMatchObject({
      name: "NetworkError",
  ...
  it("throws when robots.txt check fails", async () => {
    ...
    await expect(client.checkAndExtractUrlPreview(blockedUrl)).rejects.toMatchObject({
  ...
  it("throws when robots check passes but page 403s anyway", async () => {
    ...
    await expect(client.checkAndExtractUrlPreview("http://coinbase.com")).rejects.toMatchObject({
      name: "NetworkError",
  ...
  it("throws on URL w unsupported file extension", async () => {
    ...
    await expect(client.checkAndExtractUrlPreview("http://foo.bar/baz.pdf")).rejects.toThrow(
      InvalidUrlError,
    );
  });

...
describe.sequential("checkAndExtractUrlPreview() e2e", { timeout: 30000 }, () => {
  ...
  it("throws when robots.txt check fails", async () => {
    ...
    await expect(client.checkAndExtractUrlPreview(blockedUrl))
  ...
  it("throws when robots check passes but page 403s anyway", async () => {
    ...
    await expect(client.checkAndExtractUrlPreview("http://coinbase.com"))
      .rejects.toMatchObject({
        name: "NetworkError",
  ...
  it("throws on URL w unsupported file extension", async () => {
    ...
    await expect(client.checkAndExtractUrlPreview('http://foo.bar/baz.pdf'))

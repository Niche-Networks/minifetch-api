...
describe.sequential("x402: extractUrlContent() fails gracefully", { timeout: 30000 }, () => {
  it("throws w bad private key", async () => {
    ...
    await expect(failClient.extractUrlContent("https://anthropic.com")).rejects.toMatchObject({
      name: "NetworkError",
  ...
  it("throws on robots.txt check = blocked URL", async () => {
    ...
    await expect(client.extractUrlContent(blockedUrl)).rejects.toThrow(NetworkError);
  });
  ...
  it("throws on DNS lookup error", async () => {
    ...
    await expect(client.extractUrlContent(dnsErrUrl)).rejects.toThrow(NetworkError);
  });
  ...
  it("throws on malformed URL", async () => {
    ...
    await expect(client.extractUrlContent("vvv")).rejects.toThrow(InvalidUrlError);
  });
  ...
  it("throws on URL w unsupported file extension", async () => {
    ...
    await expect(client.extractUrlContent("http://foo.bar/baz.pdf")).rejects.toThrow(
      InvalidUrlError,
    );
  });
});

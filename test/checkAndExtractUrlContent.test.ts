...
describe.sequential("x402: checkAndExtractUrlContent() fails gracefully", { timeout: 30000 }, () => {
  ...
  it("throws w bad private key", async () => {
    ...
    const r = await expect(failClient.checkAndExtractUrlContent("https://anthropic.com")).rejects.toMatchObject({
      name: "NetworkError",
  ...
  it("throws when robots.txt check fails", async () => {
    ...
    await expect(client.checkAndExtractUrlContent(blockedUrl)).rejects.toMatchObject({
  ...
  it("throws on URL w unsupported file extension", async () => {
    ...
    await expect(client.checkAndExtractUrlContent("http://foo.bar/baz.pdf")).rejects.toThrow(
      InvalidUrlError,
    );
  });
